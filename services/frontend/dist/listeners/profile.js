...
function handleImageLocalPreview(e) {
...
function updateProfileImagePreview(avatarUrl) {
    const img = document.querySelector('#profile-image-preview');
    const placeholder = document.getElementById('profile-image-placeholder');
    if (!img || !placeholder)
        return;
...
async function loadProfile() {
    try {
        ...
        if (display2FA)
            display2FA.textContent = user.twoFa ? 'ENABLED' : 'DISABLED';
        ...
        if (authCheckbox)
            authCheckbox.checked = user.twoFa || false;
...
async function handleProfileSubmit(e) {
    ...
    const emailInput = document.querySelector('#profile-email');
    const authCheckbox = document.querySelector('#auth');
    const username = usernameInput?.value || formData.get('username');
