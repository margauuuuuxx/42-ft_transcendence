...
export async function tournamentListener() {
    ...
    window.viewTournament = async (id) => {
        try {
    ...
    function displayTournamentDetails(tournament, registrations, brackets) {
        ...
        details.innerHTML = `
            <div class="border-2 border-black p-6">
                ...
                <div class="mt-6">
                    ...
                    ${registrations.registrations?.map((r) => `<p class="mono-text">- ${r.player_alias}</p>`).join('') || '<p class="mono-text">No players registered</p>'}
                </div>
                ...
                ${brackets.brackets?.length > 0 ? `
                    <div class="mt-6">
                        ...
                        ${brackets.brackets.filter((b) => b.player2_alias !== null).map((b) => `
    ...
    window.registerPlayer = async (tournamentId) => {
    ...
    window.startTournament = async (tournamentId) => {
        try {
    ...
    window.prepareMatch = (tournamentId, player1, player2, round, position) => {
        const details = document.getElementById('tournament-details');
        if (!details)
            return;
        details.innerHTML = `
    ...
    // Player confirmation
    window.confirmPlayer = (playerNum, playerName) => {
        const match = window.currentMatch;
        if (!match)
            return;
        if (playerNum === 1) {
            match.player1Ready = true;
            const btn = document.getElementById('player1-ready');
            ...
            const btn = document.getElementById('player2-ready');
