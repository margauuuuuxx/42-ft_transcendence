...
export async function addFriendship(requester: number, addressee: number) {
...
export async function updateStatus(requester: number, addressee: number, accepted: boolean) {
  const db = await openDb();
...
export async function getUserFriendships(userId: number) {
  const db = await openDb();
