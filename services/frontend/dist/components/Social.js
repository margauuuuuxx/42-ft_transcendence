...
export class SocialPage {
    ...
    showSearchResult(message, type = 'error') {
        const searchResult = document.getElementById('searchResult');
        if (!searchResult)
            return;
    ...
    renderFriendships() {
        const pendingRequests = this.friendships.filter(f => f.status === 'pending' && !f.is_requester);
        const sentRequests = this.friendships.filter(f => f.status === 'pending' && f.is_requester);
