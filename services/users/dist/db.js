...
export async function addAvatarUrl(url, id) {
    const db = await openDb();
    const result = await db.run("UPDATE users SET avatar_url = ?  WHERE id = ?", [url, id]);
    console.log(`user id ${id} new url => ${url}`);
    await db.close();
}
export async function addUser(name, email, password) {
    const db = await openDb();
    const result = await db.run("INSERT INTO users (username, email, password_hash) VALUES (?, ?, ?)", [name, email, password]);
...
/////////////////////////////////////
export async function getUserHashPassword(email) {
    const db = await openDb();
...
export async function ModifyUserTokenVersion(id) {
    const db = await openDb();
    await db.run("UPDATE users SET token_version = token_version + 1 WHERE id = ?", id);
...
export async function updateUserLastActivity(userId) {
    const db = await openDb();
    try {
        const result = await db.run("UPDATE users SET last_active_at = datetime(CURRENT_TIMESTAMP, '+1 hour') WHERE id = ?", [userId]);
        await db.close();
        if (result.changes === 0) {
