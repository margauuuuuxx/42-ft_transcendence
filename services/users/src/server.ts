...
const setupRoutes = () => {
  ...
  server.get("/me", async (req: any, reply: any) => {
  ...
  server.patch("/me", async (req: any, reply: any) => {
     const userId = req.headers["x-user-id"];
  ...
  server.all('/users/activity', async (req: any, reply: any) => {
    const userId = req.headers['x-user-id'];
  ...
  server.post("/users/search", async (req: any, res: any) => {
  ...
  server.get('/users/:id', async (req: any, res: any) => {
  ...
  server.post("/modifyUserTokenVersion", async (req: any, res: any) => {
    const id = req.body?.id;
    if (!id) return res.code(400).send({ error: "no id in the request" });
  ...
  server.post("/register", async (request, reply) => {
    ...
    const { username, email, password } = request.body as RegisterBody; // pick specific fields

    if (!username) return reply.code(400).send({ error: "username required" });
    else if (!email) return reply.code(400).send({ error: "email required" });
    else if (!password)
