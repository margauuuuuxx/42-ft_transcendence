...
// Map of template names to their listener functions
type ListenerFunction = () => void | Promise<void>;
const listenerMap: Record<string, ListenerFunction> = {
    'login': loginListener,
...
class Router {
    ...
    addRoute(path: string, templateName: string): void {
    ...
    private async renderTemplate(templateName: string): Promise<void> {
        try {
            ...
            if (navbar && typeof (navbar as any).updateActiveLink === 'function') {
                (navbar as any).updateActiveLink();
            }
    ...
    start(): void {
        ...
        document.addEventListener('click', (e: Event) => {
            ...
            const link = target.closest('[data-navigate]') as HTMLElement;
