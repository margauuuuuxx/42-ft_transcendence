...
const setupRoutes = () => {
  server.post("/login", async (request: any, reply: any) => {
    ...
    if (user.twoFa == 1) {
      ...
      let code: string = generateCode(6);
  ...
  server.post("/send-email-code", async (req: any, res: any) => {
    interface reqBody {
      email: string;
    }
    const { email } = req.body as reqBody;
    if (!email) return res.code(400).send({ error: "email required" });
  ...
  server.post("/verify-2fa", async (request: any, reply: any) => {
    interface VerifyBody {
      email?: string;
      code?: string;
    }

    const { email, code } = request.body as VerifyBody;
  ...
  server.get("/jwt_check", async (req: any, reply: any) => {
  ...
  server.post("/refresh", async (req, res) => {
    ...
    try {
      const payload = jwt.verify(refreshToken, refreshTokenSecret) as { sub: number; version: number; iat: number; exp: number };
