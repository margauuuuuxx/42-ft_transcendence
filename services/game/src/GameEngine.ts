...
type PowerUpKind = "big_paddle" | "slow_ball";
...
type Effects = {
  left: { bigPaddleUntil: number };
  right: { bigPaddleUntil: number };
  slowBallUntil: number;
  slowBallFactor: number;
};
...
export class GameEngine {
  private canvas_size : {height : number, width : number, midH: number, midW: number};
  private ball : {x : number, y : number, vx : number, vy: number};
  private right_paddle : Paddle;
  private left_paddle : Paddle;
  private score : {left : number, right : number};
  private inputLeft: InputState;
  private inputRight: InputState;
  private callCount: number;
  private finished: boolean;
  private winner: string | null;
  private settings: EngineSettings = { ...DEFAULT_ENGINE_SETTINGS };
  private powerUps: PowerUp[];
  private lastHitter: Side | null;
  private t: number;
  private effects: Effects
  private nextSpawnAt = 0;
  private scheduleNextSpawn() { this.nextSpawnAt = this.t + this.rand(1, 5); } // temps de spawn
  private maybeSpawnPowerUp() {
    if (!this.settings.enablePowerUps) return;
    if (this.t < this.nextSpawnAt) return;
    ...
    const kind: PowerUpKind = Math.random() < 0.6 ? "big_paddle" : "slow_ball";
    this.powerUps.push({
  ...
  private applyBigPaddle(side: Side) {
    const duration = 5;
    ...
    for (const p of this.powerUps) {
      ...
      if (dx*dx + dy*dy <= rr) {
        ...
        if (p.kind === "big_paddle") {
          if (this.lastHitter) this.applyBigPaddle(this.lastHitter);
        }
  ...
  private getPaddleHeight(side: Side) {
  ...
  private randomPowerUpPos(r: number) {
    const margin = r + 20;
    const x = this.rand(margin, this.canvas_size.width - margin);
    const y = this.rand(margin, this.canvas_size.height - margin);
    return { x, y };
  }
  private expirePowerUps() {
    for (const p of this.powerUps) {
      if (!p.alive) continue;
      if (this.t >= p.expiresAt) p.alive = false;
    }
  }
  private slowActive: boolean;
  private applySlowBall() {
    const duration = 4;
  ...
  private updateSlowBallTransition() {
    const factor = this.effects.slowBallFactor;
  ...
  constructor(height: number, width: number, cfg?: Partial<{
    ballSpeed: number;
    paddleHeight: number;
    paddleSpeed: number;
  }>) {
  ...
  applySettings(s: any) {
    ...
    if (!this.settings.enablePowerUps) { this.powerUps = []; }
  }

  getInputRight(input: InputState){
    this.inputRight= input;
  }

  getInputLeft(input: InputState){
    this.inputLeft = input
  }

  updatePaddle(input: InputState, paddle: number, dt: number, midHeight: number) {
  ...
  touchPaddle(paddle: Paddle, side: Side){
    ...
    const overlapX = 
      (this.ball.vx < 0 && compare(true, paddle.x - PADDLE_WIDTH) && compare(false, paddle.x)) ||
  ...
  updateBallVector(midPaddleY: number, dir: number){
