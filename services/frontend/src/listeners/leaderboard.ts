...
export async function leaderboardListener() {
    ...
    async function loadLeaderboard() {
        try {
            ...
            const completedTournaments = data.tournaments.filter((t: any) => t.status === 'completed');
            const ongoingTournaments = data.tournaments.filter((t: any) => t.status === 'in_progress');
    ...
    async function displayCompletedTournaments(tournaments: any[]) {
                ...
                html += `
                    <div class="border-4 border-black p-6 mb-6 bg-white">
                        ...
                        <div class="border-2 border-black">
                            ...
                            ${sortedPlayers.map(([player, stats]: any, index: number) => `
    ...
    async function displayOngoingTournaments(tournaments: any[]) {
    ...
    function calculatePlayerStats(brackets: any[]) {
        const stats: any = {};
