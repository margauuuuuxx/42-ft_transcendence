...
function showVerificationError(message: string): void {
...
function clearVerificationError(): void {
    const errorDiv = document.getElementById('verification_message');
    if (errorDiv) {
        errorDiv.innerHTML = '';
 ...
 */
export function loginListener(): void {
...
function show2FAPopup(): void {
    const popup = document.getElementById('emailVerificationPopup') as any;
    if (popup && typeof popup.open === 'function') {
        popup.open();
    }
}

function hide2FAPopup(): void {
    const popup = document.getElementById('emailVerificationPopup') as any;
    ...
    if (form) {
        const input = form.querySelector('app-input[name="verification_code"]') as any;
...
function setupVerificationPopup(): void {
    const verificationForm = document.getElementById('verificationForm') as HTMLFormElement | null;
...
function setupResendButton(): void {
