...
export async function saveTwoFACode(email, code, expiresAt) {
    const db = await openDb();
    await db.run(`
...
export async function getTwoFAData(email) {
    const db = await openDb();
    return db.get("SELECT * FROM auth_data WHERE email = ?", [email]);
}
export async function clearTwoFACode(email) {
    const db = await openDb();
    return db.run(`
    UPDATE auth_data
